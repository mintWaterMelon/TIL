...
public class StudentManager {
    ...
    public void checkStudentExists(String name) {
        ...
        if (existsByName(name)) {
            System.out.println(name + " 학생은 등록되어 있습니다.");
        } else {
    ...
    public void changeStudentScore(String name, int amount) {
        if (students.isEmpty()) {
            System.out.println("등록된 학생이 없습니다.");
            return;
        }
        ...
        for (int i = 0; i < students.size(); i++) {
            ...
            if (student.getName().equals(name)) {
                ...
                int changedScore = currentScore + amount;

                try {
                    student.setScore(changedScore);
                    System.out.println("학생 점수가 변경되었습니다.");
                    System.out.println("기존 점수: " + currentScore);
                    System.out.println("변경 점수: " + changedScore);
                } catch (IllegalArgumentException e) {
                    System.out.println("변경 후 점수가 0점 이상 100점 이하가 되어야 합니다.");
                }
